...
class TestCentreRandomAugmentation(unittest.TestCase):
    def setUp(self):
        ...
        self.atom_positions = Vec3Array(x=atom_positions[:, :, 0],
                                        y=atom_positions[:, :, 1],
                                        z=atom_positions[:, :, 2])
        self.atom_mask = torch.ones(self.batch_size, self.n_atoms)

    def test_output_shape(self):
        augmented_positions = centre_random_augmentation(self.atom_positions, self.atom_mask)
    ...
    def test_random_translation(self):
        ...
        augmented_positions = centre_random_augmentation(self.atom_positions, self.atom_mask, s_trans=s_trans)
